...
function enhanceTransferUI(){
 ...
 if(grids.length){import("./idb").then(async({listFiles})=>{const records=(await listFiles()).filter(f=>f.expiresAt>Date.now()).sort((a,b)=>b.receivedAt-a.receivedAt);const batches=[];const seen=new Set();records.forEach(r=>{const batch=r.batchId||`legacy-${r.id}`;if(!seen.has(batch)){seen.add(batch);batches.push({batch,files:records.filter(x=>(x.batchId||`legacy-${x.id}`)===batch)})}});const recentBatch=batches[0]?.batch;grids.forEach(grid=>{const cards=Array.from(grid.querySelectorAll(":scope > .file-card"));cards.forEach((card,i)=>{const record=records[i];if(!record)return;const batch=record.batchId||`legacy-${record.id}`;const batchRecords=records.filter(r=>(r.batchId||`legacy-${r.id}`)===batch);const firstIndex=records.findIndex(r=>(r.batchId||`legacy-${r.id}`)===batch);card.dataset.pbBatch=batch;card.dataset.pbBatchFirst=i===firstIndex?"1":"0";card.dataset.pbBatchRecent=batch===recentBatch?"1":"0";card.removeAttribute("data-pb-batch-label");if(i===firstIndex){const recent=batch===recentBatch?"Recent Batch":"Batch";const shortSender=record.senderPeerId?String(record.senderPeerId).slice(0,6).toUpperCase():"Sender";card.dataset.pbBatchLabel=`${recent}  •  ${shortSender}  •  ${batchRecords.length} ${batchRecords.length===1?"file":"files"}`}})})}).catch(()=>{})}
