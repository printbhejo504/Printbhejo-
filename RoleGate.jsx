...
export default function RoleGate({children}){
 ...
 const load=async(s)=>{if(!s?.user||!supabase){setProfile(null);return}const {data}=await supabase.from("profiles").select("id,full_name,role,disabled,permissions").eq("id",s.user.id).maybeSingle();setProfile(data||{id:s.user.id,full_name:s.user.user_metadata?.full_name,role:"user",disabled:false,permissions:{}})};
 useEffect(()=>{if(!supabase){setSession(null);document.body.classList.remove("pb-authenticated");return}supabase.auth.getSession().then(({data})=>{const s=data.session||null;setSession(s);document.body.classList.toggle("pb-authenticated",!!s?.user);if(s)load(s)});const {data:l}=supabase.auth.onAuthStateChange((_e,s)=>{setSession(s);document.body.classList.toggle("pb-authenticated",!!s?.user);if(s)load(s);else setProfile(null)});return()=>{l.subscription.unsubscribe();document.body.classList.remove("pb-authenticated")}},[]);
