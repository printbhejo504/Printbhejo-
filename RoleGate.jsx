...
export default function RoleGate({children}){
 ...
 useEffect(()=>{if(!supabase){setSession(null);return}supabase.auth.getSession().then(({data})=>{const s=data.session||null;setSession(s);if(s)load(s)});const {data:l}=supabase.auth.onAuthStateChange((_e,s)=>{setSession(s);if(s)load(s);else setProfile(null)});return()=>l.subscription.unsubscribe()},[]);
 if(session===undefined)return <AuthGate>{children}</AuthGate>;
