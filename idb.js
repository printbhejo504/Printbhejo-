...
export async function clearFiles(){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,"readwrite");tx.objectStore(STORE).clear();tx.oncomplete=()=>{db.close();resolve();};tx.onerror=()=>{db.close();reject(tx.error);};});}
export async function deleteExpiredFiles(now=Date.now()){const files=await listFiles();const expired=files.filter(f=>f.expiresAt<=now);for(const file of expired)await deleteFile(file.id);return expired.length;}
