...
let activeOwnerUserId = "anonymous";
let ownerReady = Promise.resolve();

if (supabase) {
  ownerReady = supabase.auth.getSession().then(({ data }) => {
    activeOwnerUserId = data?.session?.user?.id || "anonymous";
  }).catch(() => {
    activeOwnerUserId = "anonymous";
  });

  supabase.auth.onAuthStateChange((_event, session) => {
    activeOwnerUserId = session?.user?.id || "anonymous";
  });
}

async function getActiveOwnerId(){
  await ownerReady;
...
export async function getBatches(ownerUserId){const owner=ownerUserId||await getActiveOwnerId();const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(BATCH_STORE,"readonly");const index=tx.objectStore(BATCH_STORE).index("ownerUserId");const req=index.getAll(IDBKeyRange.only(owner));req.onsuccess=()=>{db.close();resolve((req.result||[]).sort((a,b)=>b.createdAt-a.createdAt));};req.onerror=()=>{db.close();reject(req.error);};});}

export async function getNextBatchNumber(ownerUserId){const batches=await getBatches(ownerUserId);if(!batches.length)return 1;const active=batches.filter(b=>!b.expiresAt||b.expiresAt>Date.now());if(!active.length)return 1;return Math.max(0,...active.map(b=>Number(b.batchNumber)||0))+1;}
...
export async function saveFile(record){
  ...
  normalized.ownerUserId=normalized.ownerUserId||await getActiveOwnerId();
...
async function listAllFiles(){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,"readonly");const req=tx.objectStore(STORE).getAll();req.onsuccess=()=>{db.close();resolve(req.result||[]);};req.onerror=()=>{db.close();reject(req.error);};});}

export async function listFiles(ownerUserId){const owner=ownerUserId||await getActiveOwnerId();const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,"readonly");const index=tx.objectStore(STORE).index("ownerUserId");const req=index.getAll(IDBKeyRange.only(owner));req.onsuccess=()=>{db.close();resolve(req.result||[]);};req.onerror=()=>{db.close();reject(req.error);};});}
...
export async function clearFiles(ownerUserId){const owner=ownerUserId||await getActiveOwnerId();const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction([STORE,BATCH_STORE],"readwrite");const fileIndex=tx.objectStore(STORE).index("ownerUserId");const batchIndex=tx.objectStore(BATCH_STORE).index("ownerUserId");const fileReq=fileIndex.openKeyCursor(IDBKeyRange.only(owner));const batchReq=batchIndex.openKeyCursor(IDBKeyRange.only(owner));fileReq.onsuccess=()=>{const cursor=fileReq.result;if(cursor){tx.objectStore(STORE).delete(cursor.primaryKey);cursor.continue();}};batchReq.onsuccess=()=>{const cursor=batchReq.result;if(cursor){tx.objectStore(BATCH_STORE).delete(cursor.primaryKey);cursor.continue();}};tx.oncomplete=()=>{db.close();resolve();};tx.onerror=()=>{db.close();reject(tx.error);};});}

export async function deleteExpiredFiles(now=Date.now()){const expired=(await listAllFiles()).filter(f=>f.expiresAt<=now);for(const file of expired)await deleteFile(file.id);const allBatches=await new Promise(async(resolve,reject)=>{try{const db=await openDB();const tx=db.transaction(BATCH_STORE,"readonly");const req=tx.objectStore(BATCH_STORE).getAll();req.onsuccess=()=>{db.close();resolve(req.result||[]);};req.onerror=()=>{db.close();reject(req.error);};}catch(e){reject(e);}});for(const batch of allBatches)if(batch.expiresAt&&batch.expiresAt<=now)await removeBatch(batch.id);return expired.length;}
