...
const getDisplayName = (user) =>
  user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email?.split("@")[0] || "PrintBhejo User";
...
export default function AuthGate({ children }) {
  ...
  const permanentUrl = useMemo(() => {
    ...
    return `${base}?receiver=${encodeURIComponent(user.id)}`;
  }, [user]);
  ...
  if (!session) return (
    <div className="auth-screen">
      <div className="auth-card">
        ...
        <h1>PrintBhejo</h1>
        <p>Login karke apna permanent receiving QR use karein.</p>
        <form onSubmit={login}>
        ...
        <small>Only authorized users can login. No public signup.</small>
      </div>
    </div>
  );
  ...
  return <>
    <div className="user-bar">
      ...
      <button onClick={() => setShowQr(v => !v)}><QrCode size={17}/> My Permanent QR</button>
      <button onClick={logout}><LogOut size={17}/> Logout</button>
    </div>
    {showQr && <div className="permanent-qr-panel">
      <div><span className="qr-badge">PERMANENT QR</span><h2>{name}</h2><p>Is QR ko ek baar save/print karke rakhein. QR par aapka naam rahega.</p><code>{permanentUrl}</code></div>
