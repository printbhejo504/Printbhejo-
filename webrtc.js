...
export function createReceiverPeer({sessionId,peerId,onFile,onStatus,onPeersChange}){
  ...
  const makeReceiver=(senderPeerId)=>{
    ...
    const state={pc,remoteDescriptionSet:false,pendingIce:[],channels:new Set(),connected:false,connectedAt:null,cleanupTimer:null,iceBatcher:null};
    state.iceBatcher=createIceBatcher(sessionId,peerId,senderPeerId);
    ...
    pc.onicecandidate=e=>{if(e.candidate)state.iceBatcher.add({...e.candidate.toJSON(),toPeerId:senderPeerId,fromPeerId:peerId});else state.iceBatcher.flush();};
  ...
  return {
    ...
    addIce,
    close:()=>{peers.forEach(({pc,channels,cleanupTimer,iceBatcher})=>{if(cleanupTimer)clearTimeout(cleanupTimer);iceBatcher?.flush();channels.forEach(ch=>{try{ch.close()}catch{}});try{pc.close()}catch{}});peers.clear();notifyPeers();}
  };
}
...
export function createSenderPeer({sessionId,peerId,onStatus,onProgress}){
  ...
  const iceBatcher=createIceBatcher(sessionId,peerId,null);
